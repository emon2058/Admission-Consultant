...
db.collection('General').orderBy('Code', 'asc').onSnapshot(snapshot => {
...
function loadTableData(tableData) {
  const tableBody = document.getElementById('generalList');
  let dataHtml = '';
  let index = 0;
  for (let data of tableData) {
...
function remove(index, value) {
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      db.collection('General').doc(value.Code).delete().then(function () {
